...
export const useIpAccount = () => {
  const client = useStoryContext();
  const [loadings,setLoadings] = useState<Record<string,boolean>>({execute: false, executeWithSig: false, getIpAccountNonce: false });
  const [errors,setErrors] = useState<Record<string,string|null>>({ execute: null,executeWithSig: null,getIpAccountNonce: null });
...
const getIpAccountNonce = async (
  ...
  try {
    ...
    const response = await client.ipAccount.getIpAccountNonce(
      ipId
   );
