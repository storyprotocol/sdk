...
export const useNftClient = () => {
  const client = useStoryContext();
  const [loadings,setLoadings] = useState<Record<string,boolean>>({createNFTCollection: false });
  const [errors,setErrors] = useState<Record<string,string|null>>({ createNFTCollection: null });
