...
export const totalLicenseTokenLimitHookAbi = [
  ...
  },
  {
    ...
    outputs: [
      { name: 'totalMintingFee', internalType: 'uint256', type: 'uint256' },
    ],
    stateMutability: 'nonpayable',
  },
  {
...
export class LicenseTokenClient extends LicenseTokenReadOnlyClient {
   ...
   */
  public async approve(
    request: LicenseTokenApproveRequest,
  ): Promise<WriteContractReturnType> {
    const { request: call } = await this.rpcClient.simulateContract({
      abi: licenseTokenAbi,
      address: this.address,
...
export class TotalLicenseTokenLimitHookClient {
  ...
  constructor(
    ...
    this.address =
      address ||
      getAddress(totalLicenseTokenLimitHookAddress, rpcClient.chain?.id)
    this.rpcClient = rpcClient
    this.wallet = wallet
  }
